...
from rasa_core.channels import console, RestInput, InputChannel, BUILTIN_CHANNELS
...
def load_agent(
    core_model, interpreter, endpoints, tracker_store=None, wait_time_between_pulls=100
):
    if endpoints.model:
...
if __name__ == "__main__":
    ...
    _interpreter = NaturalLanguageInterpreter.create(cmdline_args.nlu, _endpoints.nlu)
    _agent = load_agent(
        cmdline_args.core, interpreter=_interpreter, endpoints=_endpoints
    )

    channel = BotServerInputChannel(
        _agent, preprocessor=preprocessor, port=cmdline_args.port
    )
